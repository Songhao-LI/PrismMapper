...
namespace TriangleNet
{
    public class MapDrawer : MonoBehaviour
    {
        ...
        public static void DrawMap(List<Vector2> positions,float height,Color mapColor,Color lineColor,bool needLine,float lineWidth)
        {
            ...
            Transform mapDrawer_Trans = FindObjectOfType<MapDrawer>().transform;
            go.transform.parent= mapDrawer_Trans;
            GameObject wall = new GameObject("wall");
            var wall_mf = wall.AddComponent<MeshFilter>();
            var wall_mr = wall.AddComponent<MeshRenderer>();
            wall_mr.material = mapMaterial;
            wall.transform.parent = mapDrawer_Trans;
            ...
            Instantiate(go, mapDrawer_Trans).transform.Translate(0,0,-height);
            if (needLine)
            {
                LineRenderer linerender = Instantiate(Resources.Load<GameObject>("Line"), mapDrawer_Trans).GetComponent<LineRenderer>();
                ...
                Instantiate(linerender,mapDrawer_Trans).transform.Translate(0,0,-height);
            }
